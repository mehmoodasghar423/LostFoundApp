...
const Electronics = ({ searchQuery, handleSearch, selectedType, selectedLocation, categoryselectedButton ,selectedTypeButton,selectedCityState,leftMarkerDate,rightMarkerDate}) => {
  ...
  return (
    <View style={{ height:screenHeight*0.7, marginTop: screenHeight*0.01 ,paddingBottom:screenHeight*0.01}}>
      {isLoading ? ( // Show loading icon when data is loading
        ...
        <FlatList
          data={filteredData.filter(item => item.lostItem && item.category === 'Electronics')} // Filter out items where lostItem is empty
